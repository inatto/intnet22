...
namespace intnet22.lib.associate
{
    ...
    /// </summary>
    public partial class AssociateList : IGrid
    {
        ...
        public void LoadGrid()
        {
            ...
            var sql = $@"    select * from vw_associado
                            where 1 
                            and (
                                nome like '%{search}%' or cpf like '%{search}%' or matriculaOrgao like '%{search}%' or tag_statusCarreira like '%{search}%'  or nomeInstituidorPensao like '%{search}%'
                            ) 
            ...
            while (reader.Read())
            {
                ...
                vo.VoMembroPai.VoPerson.Nome = reader["nomePai"].ToString() ?? reader["nomeInstituidorPensao"].ToString();
        ...
        {
            //
            ControlModule.OpenWindow(this, new AssociateForm(Constants.Insert, 0));
        }

        private void MainDataGrid_MouseDoubleClick(object? sender = null, MouseButtonEventArgs? e = null)
        {
            //
            ControlModule.OpenWindow(this, new AssociateForm(Constants.Update, MainDataGrid.FirstId()));
        }
